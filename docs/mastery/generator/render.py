...
def render_story_markdown(s):
    num_str = f"{s['num']:02d}"
    dep_str = ", ".join([f"Story {d:02d}" for d in s.get('depends_on', [])]) if s.get('depends_on') else "None (Entry Point)"
    unl_str = ", ".join([f"Story {u:02d}" for u in s.get('unlocks', [])]) if s.get('unlocks') else "None (Terminal Story)"
    
    lines = []
    lines.append(f"### Story {num_str} — {s['title']}")
