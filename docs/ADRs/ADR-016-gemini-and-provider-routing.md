...
## Decision
1. **Extend `AIProvider` Abstraction**: Implement [`GeminiProvider`](file:///d:/FastAPI/EstateMap/backend/app/ai/gemini_provider.py) behind the existing abstract [`AIProvider`](file:///d:/FastAPI/EstateMap/backend/app/ai/base.py) interface using Google's official `google-genai` client library with JSON schema mode and default hosted model `gemini-flash-lite-latest`.
...
### Positive
- **High Availability & Zero Single-Point-of-Failure**: If local Ollama is offline or uninstalled, requests immediately and automatically fail over to Gemini without failing user requests. If Gemini experiences quota limits or network outages, requests fail over to Ollama or rule-based heuristics.
